...
public class ReservationController {
    ...
    public ResponseEntity<ReservationDTO> add(@Valid @RequestBody ReservationDTO reservationDTO) throws ResponseStatusException, URISyntaxException {
        LOG.info("REST request to save Reservation: {}", reservationDTO);

        if (reservationDTO.getId() != null) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Id must be null to create a new Reservation: " + reservationDTO);
        }
    ...
    public ResponseEntity<?> update(@Valid @RequestBody ReservationDTO reservationDTO) throws ResponseStatusException {
        LOG.info("REST request to update Reservation : {}", reservationDTO);

        if (reservationDTO.getId() == null) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Id is null: " + reservationDTO);
        }

        reservationService.find(reservationDTO.getId())
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.BAD_REQUEST, "Reservation not found with id: " + reservationDTO.getId()));
    ...
    public ResponseEntity<?> delete(@PathVariable(value = "id") Long id) {
        ...
        reservationService.find(id)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.BAD_REQUEST, "Reservation not found with id: " + id));
