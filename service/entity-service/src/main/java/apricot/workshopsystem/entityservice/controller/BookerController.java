...
public class BookerController {
    ...
    public ResponseEntity<BookerDTO> add(@Valid @RequestBody BookerDTO bookerDTO) throws ResponseStatusException, URISyntaxException {
        LOG.info("REST request to save Booker: {}", bookerDTO);

        if (bookerDTO.getId() != null) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Id must be null to create a new Booker: " + bookerDTO);
        }
    ...
    public ResponseEntity<?> update(@Valid @RequestBody BookerDTO bookerDTO) throws ResponseStatusException {
        LOG.info("REST request to update Booker : {}", bookerDTO);

        if (bookerDTO.getId() == null) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Id is null: " + bookerDTO);
        }

        bookerService.find(bookerDTO.getId())
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.BAD_REQUEST, "Booker not found with id: " + bookerDTO.getId()));
    ...
    public ResponseEntity<?> delete(@PathVariable(value = "id") Long id) {
        ...
        bookerService.find(id)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.BAD_REQUEST, "Booker not found with id: " + id));
